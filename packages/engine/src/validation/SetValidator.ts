import type { TileInstance, TileOnBoard, TileColor } from '@rummikub/shared';
import { MIN_SET_SIZE, MAX_GROUP_SIZE, MIN_RUN_SIZE, MAX_RUN_SIZE } from '@rummikub/shared';
import { isJoker, effectiveValue, effectiveColor } from '../tile/JokerLogic.js';

// ============================================================
// 组合验证器 —— 判断一组牌是否构成合法的 Group 或 Run
// ============================================================
...
export function isValidGroup(tiles: (TileInstance | TileOnBoard)[]): SetValidationResult {
  ...
  // 提取所有牌的"有效"属性
  const nonJokers: { value: number; color: TileColor | null }[] = [];
  let jokerCount = 0;
...
export function isValidRun(tiles: (TileInstance | TileOnBoard)[]): SetValidationResult {
  const score = computeScore(tiles);
  ...
  const effective = tiles.map(t => {
    const tOnBoard = t as TileOnBoard;
    return {
...
export function validateSet(tiles: (TileInstance | TileOnBoard)[]): SetValidationResult {
  if (tiles.length < MIN_SET_SIZE) {
...
/** 计算一组牌的分数 */
export function computeScore(tiles: (TileInstance | TileOnBoard)[]): number {
  let score = 0;
  for (const tile of tiles) {
    if (isJoker(tile)) {
      // joker 按其替代值计分
      const t = tile as TileOnBoard;
