import type { TileInstance, SetOnBoard, PlayerState } from '@rummikub/shared';
...
import { validateInitialMeld, canManipulateBoard } from '../validation/MeldValidator.js';
 ...
 * 阶段1：语法验证
 */
function validateSyntax(state: GameState, moves: AtomicMove[]): MoveValidationResult {
  for (const move of moves) {
    switch (move.type) {
      case 'ADD_TILES_TO_SET':
      case 'REMOVE_TILES_FROM_SET':
      case 'DISMISS_SET':
        if (!state.boardSets.some((s: SetOnBoard) => s.id === move.setId)) {
          return { valid: false, reason: `组合 ${move.setId} 不存在` };
        }
        break;

      case 'SPLIT_SET':
        if (!state.boardSets.some((s: SetOnBoard) => s.id === move.sourceSetId)) {
      ...
      case 'MERGE_SETS':
        ...
        if (!state.boardSets.some((s: SetOnBoard) => s.id === move.targetSetId)) {
 ...
 * 计算走法批次中从手牌打出的分数
 */
function computeScoreFromBatch(state: GameState, moves: AtomicMove[]): number {
 ...
 * 获取走法批次中从手牌打出的牌
 */
function getTilesPlayedFromHand(state: GameState, moves: AtomicMove[]): TileInstance[] {
  const playerHand = state.players[state.currentPlayerIndex].handTiles;
  const handInstanceIds = new Set(playerHand.map((t: TileInstance) => t.instanceId));
  const tilesFromHand: TileInstance[] = [];
 ...
 * 未破冰的玩家不能操弄桌面牌。
 */
export function involvesBoardManipulation(moves: AtomicMove[]): boolean {
  return moves.some(m =>
    m.type === 'SPLIT_SET' ||
    m.type === 'MERGE_SETS' ||
